...
%% Load ground truth variable from Ground Truth Labeler App.
%gTruth_struct = load("./LL_Data/gTruth_LL.mat"); (for LiDAR Labeler App)
gTruth_struct = load("./GTL_Data/gTruth_GTL.mat");
gTruth = gTruth_struct.gTruth;

%% Extract data from the ground truth variable, gTruth (for Ground Truth Labeler App).
...
%a_Table = timetable2table(gTruth.LabelData);        % Convert the data-type from timetable to table (for LiDAR Labeler App)
...
%% Export to json
%saveAsJson("./savedData/label_def_LL", label_def) % (For LiDAR Labeler App)
%saveAsJson("./savedData/label_data_LL", label_data) % (For LiDAR Labele App)
